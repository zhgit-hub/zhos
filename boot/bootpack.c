...
struct FREEINFO {	/* free list node */
	unsigned int addr, size;
};

struct MEMMAN {		/* free list */
...
unsigned int memtest(unsigned int start, unsigned int end)
{
	...
	/* check cpu model 386 or over */
	eflg = io_load_eflags();
	eflg |= EFLAGS_AC_BIT; /* AC-bit = 1 */
	io_store_eflags(eflg);
	eflg = io_load_eflags();
	if ((eflg & EFLAGS_AC_BIT) != 0) { /* 386 CPU has`t AC bit,it will be reset to 0 */
		flg486 = 1;
	}
...
void memman_init(struct MEMMAN *man)
{
	man->frees = 0;			/* num of free nodes */
	man->maxfrees = 0;		/* max of frees */
	man->lostsize = 0;		/* failed to free mem size */
	man->losts = 0;			/* failed to free mem cnt */
	return;
}
	...
	for (i = 0; i < man->frees; i++) {
		if (man->free[i].size >= size) {
			...
			if (man->free[i].size == 0) {
				/* size == 0 means no mem to alloc */
				man->frees--;
				for (; i < man->frees; i++) {
					man->free[i] = man->free[i + 1]; /* free node move forward */
				}
			}
			return a;
		}
	}
	return 0; /* no mem to alloc */
}

int memman_free(struct MEMMAN *man, unsigned int addr, unsigned int size)
{
	int i, j;
	/* decide where to put free mem */
	for (i = 0; i < man->frees; i++) {
	...
	if (i > 0) {
		/* if it can merge with pre free node */
		if (man->free[i - 1].addr + man->free[i - 1].size == addr) {
			man->free[i - 1].size += size;
			if (i < man->frees) {
				/* if it can merge with next free node */
	...
	/* can`t merge with around */
	if (man->frees < MEMMAN_FREES) {
		/* free node move backward */
		for (j = man->frees; j > i; j--) {
			man->free[j] = man->free[j - 1];
		}
		man->frees++;
		if (man->maxfrees < man->frees) {
			man->maxfrees = man->frees; /* update max free nodes */
		}
		man->free[i].addr = addr;
		man->free[i].size = size;
		return 0;
